...
TrajectoryCost::TrajectoryCost(
  ...
  // 未来每个时间点计算方法
  // FLAGS_prediction_total_time---prediction模块中障碍物轨迹预测总时间，5s
  const float total_time = std::min(heuristic_speed_data_.TotalTime(), FLAGS_prediction_total_time);
// eval_time_interval是prediction模块中障碍物两个预测轨迹点的时间差，0.1s ----------- 差不多50个位置点
...
ComparableCost TrajectoryCost::CalculatePathCost(
  ...
  for (float curve_s = 0.0; curve_s < (end_s - start_s);
       curve_s += config_.path_resolution()) {// path_resolution:1.0m,每1m采样一个点
    const float l = curve.Evaluate(0, curve_s); // 根据拟合的5次多项式，计算该点的侧方距离

    path_cost += l * l * config_.path_l_cost() * quasi_softmax(std::fabs(l));  // A.侧方距离l的cost，path_l_cost：6.5
    ...
    const float dl = std::fabs(curve.Evaluate(1, curve_s));// B.侧方速度cost，path_dl_cost：8000
    path_cost += dl * dl * config_.path_dl_cost();

    const float ddl = std::fabs(curve.Evaluate(2, curve_s));
    path_cost += ddl * ddl * config_.path_ddl_cost();  // C.侧方加速度cost,path_ddl_cost:5
  }
...
结论：动态障碍物计算需要考虑到每个时间点上障碍物的位置，每个时间点每个障碍物的cost计算分为碰撞cost和风险cost
*/ 
...
/* 两两node之间的cost计算
两两level中不同node之间cost计算分为三个部分：路径开销PathCost，
静态障碍物开销StaticObstacleCost和动态障碍物开销DynamicObstacleCost，从代码中我们可以清晰的开导cost的组成。
*/ 
