 ...
 * SpeedOptimizer类继承自Task类，实现了继承来的Execute()函数，
 * 又在Execute()中调用了其纯虚成员函数Process()。
 * 而DpStSpeedOptimizer类又继承了SpeedOptimizer类，并实现了Process()函数，
 * 在Process()中调用了成员函数SearchStGraph()，到这里，才算进入了使用动态规划算法进行速度规划的正题。
 **/
...
/*
首先构造了DpStGraph类实例，该类包含了所有可能的(s,t)节点的代价表，
应用动态规划算法查找出代价最小的路径，然后计算速度，就得到了速度规划的结果 。
*/ 
...
Status DpStSpeedOptimizer::Process(const SLBoundary& adc_sl_boundary,
  ...
  if (!SearchStGraph(boundary_mapper, speed_limit_decider, path_data,speed_data, path_decision, st_graph_debug)) {
