...
PathTimeGraph::PathTimeGraph(
  ...
  init_d_ = init_d;
// 在构造函数中，给所有s-t graph添加所有的动静态障碍物
...
//对静态障碍物的占据范围，按s坐标升序排列
  ...
  for (auto& path_time_obstacle : path_time_obstacle_map_) {
    double s_upper = std::max(path_time_obstacle.second.bottom_right().s(),path_time_obstacle.second.upper_right().s());
    double s_lower = std::min(path_time_obstacle.second.bottom_left().s(),path_time_obstacle.second.upper_left().s());
    path_time_obstacle.second.set_path_lower(s_lower);
    path_time_obstacle.second.set_path_upper(s_upper);

    double t_upper = std::max(path_time_obstacle.second.bottom_right().t(),path_time_obstacle.second.upper_right().t());
    double t_lower = std::min(path_time_obstacle.second.bottom_left().t(),path_time_obstacle.second.upper_left().t());
...
// 在s-t graph中画出矩形，代表静态障碍物
// 参数discretized_ref_points 用于障碍物的（x-y）到frenet转换
...
// 获取s&l方向的起点和终点，多边形在x-y，
// 因此参考线用于将多边形转换为frenet
...
// 在s-t grap中画出静态障碍物占据的网格图，实际是一个矩形，
  path_time_obstacle_map_[obstacle_id].set_obstacle_id(obstacle_id);
  path_time_obstacle_map_[obstacle_id].mutable_bottom_left()->CopyFrom(SetPathTimePoint(obstacle_id, sl_boundary.start_s(), 0.0));
  path_time_obstacle_map_[obstacle_id].mutable_bottom_right()->CopyFrom(SetPathTimePoint(obstacle_id, sl_boundary.start_s(),
                       FLAGS_trajectory_time_length));
  path_time_obstacle_map_[obstacle_id].mutable_upper_left()->CopyFrom(etPathTimePoint(obstacle_id, sl_boundary.end_s(), 0.0));
...
void PathTimeGraph::SetDynamicObstacle(
  ...
  while (relative_time < time_range_.second) {
    ...
    if (sl_boundary.start_s() > path_range_.second ||
        ...
        sl_boundary.start_l() > left_width ||
        sl_boundary.end_l() < -right_width) {
      // 如果障碍物之前在障碍地图_中，现在它消失或转到另一条车道，中断并返回，所以绘制平行四边形完成
      if (path_time_obstacle_map_.find(obstacle->Id()) != path_time_obstacle_map_.end()) {
        break;
      } else {
      // 如果之前在障碍地图_中从未观察到障碍物，添加时间看它会不会出现
        relative_time += FLAGS_trajectory_time_resolution;
        continue;
      }
    }
//如果障碍物是第一次被观察到，则将其添加到障碍物地图_，
//这个只执行一次
    if (path_time_obstacle_map_.find(obstacle->Id()) ==path_time_obstacle_map_.end()) {
