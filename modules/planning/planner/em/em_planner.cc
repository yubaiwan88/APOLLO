...
void EMPlanner::RegisterTasks() {
  task_factory_.Register(DP_POLY_PATH_OPTIMIZER , []() -> Task* { return new DpPolyPathOptimizer(); });
  ...
  task_factory_.Register(POLY_ST_SPEED_OPTIMIZER,[]() -> Task* { return new PolyStSpeedOptimizer(); });
}

/*
1、注册各种任务类（目前已注册：交通决策器TrafficDecider、动态规划多项式路径优化器DpPolyPathOptimizer、路径决策器PathDecider、
        动态规划ST坐标速度优化器DpStSpeedOptimizer、速度决策器SpeedDecider、二次规划样条路径优化器QpSplinePathOptimizer、
        二次规划样条ST坐标速度优化器QpSplineStSpeedOptimizer）；
2、使用工厂创建模式生成具体的任务类对象（通过查询配置文件modules/planning/conf/planning_config.pb.txt获知，
        任务类型包括： TRAFFIC_DECIDER、 DP_POLY_PATH_OPTIMIZER、PATH_DECIDER、DP_ST_SPEED_OPTIMIZER、
        SPEED_DECIDER、QP_SPLINE_ST_SPEED_OPTIMIZER，
        实际生成的任务对象分别为：TrafficDecider、DpPolyPathOptimizer、PathDecider 、DpStSpeedOptimizer、SpeedDecider、
        QpSplineStSpeedOptimizer类对象。注意：任务对象的创建可根据修改配置文件动态调整）；
3、最后完成各种任务类对象的初始化。
*/ 
// 配置EM算法中的优化器（任务）
Status EMPlanner::Init(const PlanningConfig& config) {
  // AINFO << "In EMPlanner::Init()";
  // 注册任务，任务里包含优化器，向任务容器中通过配置文件加入具体的优化器
RegisterTasks();
  for (const auto task : config.em_planner_config().task()) {
    // emplace_back向容器插入数据，这里插入em_task
    // 2、使用工厂创建模式生成具体的任务类对象
...
/*
1、循环遍历参考线，首先对当前参考线信息（reference_line_info，即候选路径）作出判断，若其非向前直行，则加入10000的代价（cost）；
2、接下来，基于起始点和当前参考线信息，调用EMPlanner::GenerateInitSpeedProfile函数生成初始速度，
        若失败则继续调用EMPlanner::GenerateSpeedHotStart生成初始速度；
3、之后是整个函数的核心，针对当前帧frame和参考线信息reference_line_info，
4、逐一调用任务类对象数组tasks_里的优化器（当前配置文件按照以下顺序使用以下决策或优化器：TrafficDecider、DpPolyPathOptimizer、
      PathDecider 、DpStSpeedOptimizer、SpeedDecider、QpSplineStSpeedOptimizer）进行最优估计，计算当前参考线的最优路径和速度。
      若针对当前参考线无法计算出最优路径和速度，则将其代价值设为无穷大，表示当前参考线无法使用；
      若对当前参考线可计算出最优路径和速度，则调用ReferenceLineInfo::CombinePathAndSpeedProfile函数合并路径和速度信息，
5、最后调用ReferenceLineInfo::setTrajectory函数更新当前参考线的轨迹，返回给上一级Planning::Plan函数使用。

整个函数存在很多输出调试信息的语句，注意不要被他们所干扰，影响对核心思路的理解。
*/ 
/*
1、循环遍历候选参考线（依赖高精度地图和Routing模块给出的全局规划结果），根据起始点和参考线信息，调用初速度
2、针对当前帧和参考线信息-----逐一调用任务类对象数组里的优化器进行最优评估，计算当前参考线的最优路径和速度。
        若成功，通过合并函数合并路径和速度信息
3、通过函数更新当前参考线的轨迹
*/ 
...
Status EMPlanner::Plan(const TrajectoryPoint& planning_start_point,Frame* frame) {
  ...
  bool disable_low_priority_path = false;  //禁用低优先级路径
  ...
  for (auto& reference_line_info : frame->reference_line_info()) {
    ...
    // 根据候选参考线和当前点和frame中包含的信息，通过PlanOnReferenceLine（）函数进行轨迹规划 ------得到规划的状态
...
/*
1、首先对当前参考线信息（reference_line_info，即候选路径）作出判断，若其非向前直行，则加入10000的代价（cost）；
2、接下来，基于起始点和当前参考线信息，调用EMPlanner::GenerateInitSpeedProfile函数生成初始速度，
        若失败则继续调用EMPlanner::GenerateSpeedHotStart生成初始速度；
3、之后是整个函数的核心，针对当前帧frame和参考线信息reference_line_info，
4、逐一调用任务类对象数组tasks_里的优化器（当前配置文件按照以下顺序使用以下决策或优化器：TrafficDecider、DpPolyPathOptimizer、
...
Status EMPlanner::PlanOnReferenceLine(const TrajectoryPoint& planning_start_point, Frame* frame,ReferenceLineInfo* reference_line_info) 
{
  ...
  auto ret = Status::OK();
// 逐一调用任务类对象数组tasks_里的优化器进行最优估计，计算当前参考线的最优路径和速度
...
// 若对当前参考线可计算出最优路径和速度，则调用ReferenceLineInfo::CombinePathAndSpeedProfile函数合并路径和速度信息，
  ...
  if (!reference_line_info->CombinePathAndSpeedProfile(planning_start_point.relative_time(),
    ...
    std::string msg("Fail to aggregate planning trajectory."); //无法聚合规划轨迹
    AERROR << msg;
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }
...
// 更新当前参考线的轨迹，返回给上一级Planning::Plan函数使用。
...
/*
1、首先获取上一帧last_frame的参考线信息last_reference_line_info；
2、之后，对当前参考线信息reference_line_info调用ReferenceLineInfo::IsStartFrom函数
       来判断其是否起始于上一帧参考线信息last_reference_line_info，若不是直接返回，否则继续；
3、接着，基于上一帧参考线的速度信息生成当前帧参考线的速度信息，注意这里用到了(x, y)坐标到(s, l)坐标的转换。
*/ 
// 得到初始速度
...
// 这是一个简单的虚拟热启动，需要以后完善
// 将初始点的速度限制在[5.0，FLAGS_planning_upper_speed_limit]范围内，并为当前帧参考线的初始速度信息。
std::vector<SpeedPoint> EMPlanner::GenerateSpeedHotStart(const TrajectoryPoint& planning_init_point) {
...
SLPoint EMPlanner::GetStopSL(const ObjectStop& stop_decision,const ReferenceLine& reference_line) const {
  SLPoint sl_point;
