...
Status LatticePlanner::PlanOnReferenceLine(
  ...
  // 1、获得当前参考线（车辆未来轨迹），通过ToDiscretizedReferenceLine进行参考线的离散化
  ...
  // 2、使用规划原点匹配第一步离散化后的轨迹，找到匹配点matched_point
  PathPoint matched_point = PathMatcher::MatchToPath( *ptr_reference_line, planning_init_point.path_point().x(),
  ...
  //  加入感知信息，将之前感知模块内容加入到path_time_graph,也就是ST-graph
  //  具体参数包括障碍物筛选，去掉与车辆未来轨迹不发生冲突的障碍物，设置动态障碍物
  //   planning_target判断,即判断是否有停车点，如果有的话打印出停车点的s值
  ...
  if (planning_target.has_stop_point()) {
    ADEBUG << "Planning target stop s: " << planning_target.stop_point().s() << "Current ego s: " << init_s[0];
  }
  ...
  //  5、生成横纵向轨迹，分别生成纵向和横向的一维轨迹束
  /*
  涉及的函数很多，具体包括纵向轨迹生成以及横向轨迹生成，
  通过设置初始状态、末状态，通过五次多项式拟合，得出相关系数，再通过求导，即可得出速度，再求导得出加速度。
  再求导得出的加加速度，也就是Jerk，这是用来评价轨迹的舒适度的变量。
  */
  ...
  6、首先，根据动态约束条件对一维轨迹的可行性进行评估。
         第二，评估可行的纵向和横向轨迹对，并根据成本进行排序。
  */
//  轨迹评价函数：上一步生成许多条轨迹，根据cost进行排序
  ...
  while (trajectory_evaluator.has_more_trajectory_pairs()) {
    ...
    // put combine trajectory into debug data    -----------------将合并轨迹放入调试数据
    ...
    if (AdapterManager::GetLocalization() == nullptr) {
      ...
      // 1. Get future trajectory from localization -----从定位中获取未来轨迹
      DiscretizedTrajectory future_trajectory = GetFutureTrajectory();
      // 2. Map future trajectory to lon-lat trajectory pair -----将未来轨迹映射到横向-纵向轨迹对
    ...
    // Print the chosen end condition and start condition --------打印所选的结束条件和开始条件
    ADEBUG << "Starting Lon. State: s = " << init_s[0] << " ds = " << init_s[1]  << " dds = " << init_s[2];
    // cast     --------------投射
