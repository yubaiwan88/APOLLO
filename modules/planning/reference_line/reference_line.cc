...
// 2种方式生成参考线，可以通过一组参考点来生成，也可以通过地图路径来生成
// 通过一组参考点生成，reference_points_直接拷贝赋值了，然后再用reference_points生成hdmap::MapPathPoint，最后保存到map_path_。
ReferenceLine::ReferenceLine(
    const std::vector<ReferencePoint>& reference_points)
    : reference_points_(reference_points),
      map_path_(std::move(std::vector<hdmap::MapPathPoint>(reference_points.begin(), reference_points.end()))) {
...
bool ReferenceLine::Stitch(const ReferenceLine& other) {
  ...
  bool first_join = first_sl.s() > 0 && first_sl.s() < other.Length() && std::fabs(first_sl.l()) < kStitchingError;
  // 找到终点的交点
  ...
  bool last_join = last_sl.s() > 0 && last_sl.s() < other.Length() &&std::fabs(last_sl.l()) < kStitchingError;
  ...
  if (last_join) {
    ...
    reference_points_.insert(reference_points_.end(),other_points.begin() + end_i, other_points.end());
  }
  map_path_ = MapPath(std::move(std::vector<hdmap::MapPathPoint>(reference_points_.begin(), reference_points_.end())));
  return true;
}
...
// 收缩参考线，就是删除查询距离1.5倍之后的ReferencePoint
bool ReferenceLine::Shrink(const common::math::Vec2d& point,double look_backward, double look_forward) {
  common::SLPoint sl;
  if (!XYToSL(point, &sl)) {
  ...
  if (sl.s() + look_forward < Length()) { //查询收缩上届
