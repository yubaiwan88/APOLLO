...
//这个函数的逻辑比较简单，主要是要确定lower_bound和upper_bound中都是有效值，如果存在inf,返回false
  ...
  A的列数是(x_knots_.size() - 1) * num_params。
  每一段curve的列是num_params，inequality_constraint包含所有的curve,
  因此是(x_knots_.size() - 1) * num_params；x_knots_--每一段curve起点处s坐标的集合。
*/
  ...
  for (uint32_t i = 0; i < filtered_lower_bound.size(); ++i) {
    ...
    const double corrected_x = filtered_lower_bound_x[i] - x_knots_[index];//对输入的相对curve起点的s坐标
    double coef = 1.0;
...
// 添加某个点的函数值约束，（可能是添加的是每段curve起始点的约束）
